...
def relu_vector(vec, offset):
    ...
    return concat_list(
        [
            select(d[-1], falsecase=d, truecase=Const(0, len(d)))[
                24 - offset : 32 - offset
            ]
            for d in vec
        ]
...
def act_top(start, start_addr, dest_addr, nvecs, func, accum_out):
    ...
    rtl_assert(
        ~(start & busy),
        Exception(
            "Dispatching new activate instruction while previous instruction is still running."
        ),
    )
    ...
    act_out = mux(
        act_func, invals, relu_vector(accum_out, 24), sigmoid_vector(accum_out), invals
    )
