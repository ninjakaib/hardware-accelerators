...
def fetch_json_reports():
    ...
    if not os.path.exists(SOURCE_DIR):
        ...
        print(
            "Please run the organize_verilog.py script first to create the 'all' directory."
        )
        return 0
    ...
    if not os.path.exists(WSL_LOGS_PATH):
        print(
            f"Error: WSL logs path '{WSL_LOGS_PATH}' does not exist or is not accessible."
        )
    ...
    for design in designs:
        ...
        report_source_path = os.path.join(
            WSL_LOGS_PATH, design, "base", "6_report.json"
        )
...
def fetch_finish_reports():
    ...
    if not os.path.exists(WSL_REPORTS_PATH):
        print(
            f"Error: WSL reports path '{WSL_REPORTS_PATH}' does not exist or is not accessible."
        )
        print("Please make sure the WSL path is correct and accessible from Windows.")
        return 0
    ...
    for design in designs:
        ...
        report_source_path = os.path.join(
            WSL_REPORTS_PATH, design, "base", "6_finish.rpt"
        )
...
def find_max_arrival_times():
    ...
    for file_path in report_files:
        ...
        if arrival_times:
            ...
            # Add to data list
            data.append({"filename": file_name, "max_arrival_time": max_abs_time})
        else:
    ...
    if data:
        ...
        print("\nResults:")
        print(
            f"Maximum absolute data arrival time: {df_sorted['max_arrival_time'].max():.6f}"
        )
        ...
        df_sorted["index_name"] = df_sorted["filename"].str.replace(
            "_finish.rpt", "_report"
        )
...
def main():
    ...
    output_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "analysis_data.csv"
    )
