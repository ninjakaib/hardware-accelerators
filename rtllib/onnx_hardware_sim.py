...
class ONNXHardwareSimulator:
    def __init__(self, model_path: str, operator_mapping: Dict[str, Callable]):
        # Load ONNX model
    ...
    def setup_memories(self):
        ...
        for init in self.graph.initializer:
            ...
            mem = pyrtl.MemBlock(
                bitwidth=8,  
                addrwidth=len(np_array.flatten()),
                name=init.name
            )
    ...
    def _get_initializer(self, name: str) -> pyrtl.WireVector:
...
def test_simulator():
    ...
    sim = ONNXHardwareSimulator("data/mnist-12.onnx", op_mapping)
