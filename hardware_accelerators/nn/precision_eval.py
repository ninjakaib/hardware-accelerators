...
def detailed_precision_comparison(fp32_model_path, trials=3, batch_sizes=[1, 16, 32, 64, 128, 256]):
    ...
    for precision in precisions:
        ...
        for batch_size in batch_sizes:
            ...
            for trial in range(trials):
                ...
                print(f" done. Time: {result['inference_time']:.4f}s, Accuracy: {result['accuracy']:.2f}%")
            
            # Calculate averages
            avg_accuracy = sum(batch_results["accuracy"]) / len(batch_results["accuracy"])
            avg_time = sum(batch_results["inference_time"]) / len(batch_results["inference_time"])
    ...
    print("\nComparison Results")
    print("=" * 80)
    print(f"{'Batch Size':^10} | {'FP32 Acc':^10} | {'BF16 Acc':^10} | {'Acc Diff':^10} | {'FP32 Time':^10} | {'BF16 Time':^10} | {'Speedup':^10}")
    print("-" * 80)
    ...
    for batch_size in batch_sizes:
        ...
        print(f"{batch_size:^10} | {fp32_acc:^10.2f} | {bf16_acc:^10.2f} | {acc_diff:^10.2f} | {fp32_time:^10.4f} | {bf16_time:^10.4f} | {speedup:^10.2f}x")
    
    # Calculate and print overall averages
    avg_acc_diff = sum(all_results["bf16"][bs]["avg_accuracy"] - all_results["fp32"][bs]["avg_accuracy"] for bs in batch_sizes) / len(batch_sizes)
    avg_speedup = sum(all_results["fp32"][bs]["avg_inference_time"] / all_results["bf16"][bs]["avg_inference_time"] for bs in batch_sizes) / len(batch_sizes)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Compare model inference with FP32 vs BF16")
    parser.add_argument(
        "--model_path", 
        type=str,
